...
import { AuthShell, Field, SubmitButton, AuthNotice, mapAuthError } from "./auth-shared";
      ...
      {sent ? (
        <div className="space-y-3 text-[0.86rem] text-content-secondary">
          <AuthNotice variant="success">
            If an account exists for <span className="text-content-primary">{email}</span>, a reset link is
          ...
          {import.meta.env.DEV && devToken && (
            <p className="rounded-xl bg-white/[0.04] px-3 py-2 text-[0.8rem]">
              Dev:{" "}
              <Link
                className="text-content-primary underline underline-offset-4"
